...
bool OMETiffReader::open(const QString &filename)
{
    ...
    try {
        if (filename.endsWith(".ome.tiff", Qt::CaseInsensitive) ||
            ...
            std::shared_ptr<ome::xml::meta::MetadataStore> metaStore = std::make_shared<ome::xml::meta::OMEXMLMetadata>();
...
static double floatQuantityToNm(const ome::xml::model::primitives::Quantity<ome::xml::model::enums::UnitsLength, ome::xml::model::primitives::PositiveFloat >& q)
{
    double value = q.getValue();
    auto unit = q.getUnit();
    if (unit == ome::xml::model::enums::UnitsLength::MICROMETER) value *= 1000.0;
    else if (unit == ome::xml::model::enums::UnitsLength::MILLIMETER) value *= 1000000.0;
    else if (unit == ome::xml::model::enums::UnitsLength::NANOMETER) { /* already nm */ }
    else if (unit == ome::xml::model::enums::UnitsLength::METER) value *= 1e9;
    return value;
}
...
ImageMetadata OMETiffReader::extractMetadata(dimension_size_type imageIndex) const
{
    ...
    try {
        ...
        int bytesPerPixel = 1;
        if (pt == enums::PixelType::UINT16 || pt == enums::PixelType::INT16) bytesPerPixel = 2;
        else if (pt == enums::PixelType::UINT32 || pt == enums::PixelType::INT32 || pt == enums::PixelType::FLOAT) bytesPerPixel = 4;
        else if (pt == enums::PixelType::DOUBLE) bytesPerPixel = 8;
        meta.dataSizeBytes = static_cast<size_t>(meta.sizeX) * meta.sizeY * meta.sizeZ * meta.sizeC * meta.sizeT * bytesPerPixel;
    } catch (const std::exception &e) {
    ...
    try {
        ...
        for (dimension_size_type ch = 0; ch < channelCount; ++ch) {
            ...
            try {
                ...
                // Convert to nm
                if (unit == enums::UnitsLength::MICROMETER) value *= 1000.0;
                else if (unit == enums::UnitsLength::METER) value *= 1e9;
